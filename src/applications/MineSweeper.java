...
public class MineSweeper extends JPanel implements AWTEventListener, ActionListener {
	...
	private void showAbout() {
		JOptionPane.showMessageDialog(this, "<html>Author : Aniruddha Dutta Chowdhury <br>Version : 1.0</html>",
	...
	private boolean checkGameState() {
		...
		if (isWin) {
			...
			JOptionPane.showMessageDialog(this, "You win the game :D", "Congrats", JOptionPane.INFORMATION_MESSAGE,
					null);
		}
		return isWin;
	}
	...
	private void updateSurroundingZeros(Point currentPoint) {
		...
		for (Point p : points) {
			...
			if (b != null && b.getBombCount() == 0 && b.getState() != State.Clicked && b.getState() != State.Marked
					&& b.isBomb() == false) {
	...
	private Point[] getSurroundings(Point cPoint) {
		...
		Point[] points = { new Point(cX - 1, cY - 1), new Point(cX - 1, cY), new Point(cX - 1, cY + 1),
				new Point(cX, cY - 1), new Point(cX, cY + 1), new Point(cX + 1, cY - 1), new Point(cX + 1, cY),
				new Point(cX + 1, cY + 1) };
		return points;
	}
